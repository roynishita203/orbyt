  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const dedupedPlans = Array.from(new Map(events.plansCreated.map((p) => [String(p.planId), p])).values())
        setAvailablePlans(dedupedPlans)

        knownSubscriptionIds = subscriber
          ? events.subscriptionsCreated.filter((s) => s.subscriber === subscriber).map((s) => s.subscriptionId)
          : []
